...
**Inherits:**
[IEOFeedManager](/src/interfaces/IEOFeedManager.sol/interface.IEOFeedManager.md), OwnableUpgradeable, PausableUpgradeable

The EOFeedManager contract is responsible for receiving feed updates from whitelisted publishers. These
...
## State Variables
### _priceFeeds
*Map of feed id to price feed (feed id => PriceFeed)*
...
*Map of whitelisted publishers (publisher => is whitelisted)*
...
### _supportedFeedIds
*Map of supported feeds, (feed id => is supported)*
...
### _pauserRegistry
Address of the `PauserRegistry` contract that this contract defers to for determining access control
(for pausing).
...
*Allows only whitelisted publishers to call the function*
...
*Allows only non-zero addresses*
...
*The feed verifier contract must be deployed first*
