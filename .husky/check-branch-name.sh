...
if [[ ! ${branch_name} =~ ${regular_branch_pattern} && ! ${branch_name} =~ ${hotfix_branch_pattern} && ! ${branch_name} =~ ${release_candidate_branch_pattern} && ! ${branch_name} =~ ${develop_branch_pattern} && ! ${branch_name} =~ ${main_branch_pattern} ]]; then
