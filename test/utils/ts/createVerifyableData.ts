...
async function run() {
  ...
  const leafInputs1: ILeafInput[] = [
    ...
    const unhashedLeaf = abiCoder.encode(["uint256", "uint256", "uint64"], d);
    return {
      leafIndex: i + 1,
      unhashedLeaf,
    };
  ...
  const tree1 = new MerkleTree(
    leafInputs1.map((d) => ethers.keccak256(d.unhashedLeaf)),
    ethers.keccak256,
    { sort: true },
  );
  ...
  const tree2 = new MerkleTree(
    leafInputs2.map((d) => ethers.keccak256(d.unhashedLeaf)),
    ethers.keccak256,
    { sort: true },
  );
  ...
  const leafInputs3: ILeafInput[] = [[1, 9999999999, 20398409484]].map((d, i) => {
    const unhashedLeaf = abiCoder.encode(["uint256", "uint256", "uint64"], d);
    return {
      leafIndex: i + 1,
      unhashedLeaf,
  ...
  const tree3 = new MerkleTree(
    leafInputs3.map((d) => ethers.keccak256(d.unhashedLeaf)),
    ethers.keccak256,
    { sort: true },
  );
  ...
  let _sig1: mcl.G1 = new mcl.G1(),
    _sig2: mcl.G1 = new mcl.G1(),
    _sig3: mcl.G1 = new mcl.G1();
  let msg1 = ethers.keccak256(
    abiCoder.encode(
      ["bytes32", "uint64", "bytes32", "uint32", "address"],
      [root1, block1, blockHash1, chainId, aggregator],
    ),
  );
  let msg2 = ethers.keccak256(
    ethers.solidityPacked(
      ["bytes32", "uint64", "bytes32", "uint32", "address"],
      [root2, block2, blockHash2, chainId, aggregator],
    ),
  );
  let msg3 = ethers.keccak256(
    ethers.solidityPacked(
      ["bytes32", "uint64", "bytes32", "uint32", "address"],
      [root3, block3, blockHash3, chainId, aggregator],
    ),
  );
